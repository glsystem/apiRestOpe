    ...
    constraint pk_id_nome PRIMARY KEY(id, nome_completo)
	);
	...
	CONSTRAINT fk_medida FOREIGN KEY (id_medida) REFERENCES medida(id) 
	);
    ...
    select cpf, senha from glsystem.login as l
    inner join glsystem.funcionario as f on
    f.id = l.id_funcionario;
    
    select * from glsystem.funcionario;
    select * from glsystem.produto;
